...
export function provideAutocomplete() {
	let ret = {
		onDidInsertSuggestion (arg) {
			...
			if (arg.suggestion.leftLabel == "object" || arg.suggestion.leftLabel == "array") {
				// get the buffer history and check that the extra quotation mark created by the
				// auto insert package thingy, isn't present.
				let buffer = arg.editor.getBuffer()
				let end = buffer.historyProvider.undoStack[buffer.historyProvider.undoStack.length-1].patch.getChanges()[0].newEnd
				let range = [end,[end.row,end.column+1]];
				// if the snipped ends at the end of the file there cannot be a quotation mark
				if (!buffer.getEndPosition().isEqual([end.row,end.column+1])) {
					// if the extra quotation mark is present, order 66 it.
					if (buffer.getTextInRange(range) == "\"") {
						buffer.delete([end,[end.row,end.column+1]])
						// grouping last change so that the quotation mark isn't a separate
						// undo
						buffer.historyProvider.groupLastChanges();
					}
				}
			}
		},
		selector : '.source.sbjson',
		inclusionPriority : 0,
