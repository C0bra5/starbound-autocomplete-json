...
export function provideAutocomplete() {
	let ret = {
		...
		getSuggestions : function(options) {
			...
			//get suggestions
			let suggestions = startSearch(fileExt,getLocation(buffer,bufferPosition));
...
function getLocation(buffer, bufferPosition) {
	...
	text = text.replace(/((.|\r|\n)+?)\/\/([^\r\n]+)/,function(fullMatch,$1,$2,$3){
		if (($1.replace(/[^"]/g,"").length % 2) === 0) {
			return $1;
		} else {
			//the // was in a string, just temporarely modify it so that we can fix it back later
			return $1+"ΓΓΓΓΓΓ"+$3;
		}
	})
	while (text.match(/((.|\r|\n)+?)\/\/([^\r\n]+)/)){
	...
	//getrid of useless array strings
	text = text.replace(/\"[^\"]*\"\,/g,"");
